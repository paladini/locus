...
public class CursoMBean {
	...
	public String avancar() {
		...
		if (lista.size() == 0) {
			FacesContext context = FacesContext.getCurrentInstance();
			context.addMessage(null, new FacesMessage(
					FacesMessage.SEVERITY_WARN, "Adicione um curso!",
					"Antes de prosseguir, adicione pelo menos um curso."));
			return null;
		} else {
			return "4-tela-concluidas?faces-redirect=true";
		}
	...
	public void setSelecionado(Curso selecionado) {
		...
		if (selecionado != null) {
			...
			for (int i = 0; i < listaDisciplinasDoCurso.size(); i++) {
				vetorDisciplinasDoCurso[i] = listaDisciplinasDoCurso.get(i)
						.getNome();
			}
